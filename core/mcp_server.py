...
async def list_tools() -> list[Tool]:
    return [
        Tool(name="get_swarm_summary",  description="Returns overall swarm PnL, equity, trade count and status.", inputSchema={"type": "object", "properties": {}}),
        Tool(name="list_agents",        description="Lists all agents with id, type, symbol, equity, status.",     inputSchema={"type": "object", "properties": {}}),
        Tool(name="halt_swarm",         description="Immediately halt all trading.", inputSchema={"type": "object", "properties": {"reason": {"type": "string"}}}),
        Tool(name="resume_swarm",       description="Resume trading after halt.",    inputSchema={"type": "object", "properties": {}}),
        Tool(name="pause_agent_type",   description="Pause all agents of a given type.", inputSchema={
            "type": "object",
            "properties": {"agent_type": {"type": "string", "enum": ["SCALPER","SWING","NEWS_REACTIVE","HEDGER"]}},
            "required": ["agent_type"],
        }),
        Tool(name="get_agent_metrics",  description="Get metrics for a specific agent.", inputSchema={
            "type": "object",
...
async def call_tool(name: str, arguments: dict) -> list[TextContent]:
    ...
    if name == "get_swarm_summary":
        ...
        data = [{"id": a.agent_id, "type": a.agent_type.value, "symbol": a.symbol.value,
                 "equity": a.equity, "status": a.status.value}
                for a in orch._agents.values()] if orch else []
